...
def classify_file(qdir, base_dir, par_dir, tax_dir, dist_scaling, run_id, train_eval):
    ...
    base_embeddings = jnp.array(np.load(base_dir)["embeddings"])
    print("base embeddings.shape: ", base_embeddings.shape)

    print("Reading query embeddings...")
    query_embeddings = jnp.array(np.load(qdir)["embeddings"])
    ...
    for i in tqdm(range(query_embeddings.shape[0]), desc="Classifying queries", file=sys.stderr, dynamic_ncols=True, mininterval=5):
        query = query_embeddings[i]
        q_dist = model.seq_dist(query, base_embeddings)

        if train_eval and i < base_embeddings.shape[0]:
            new_n2s, new_node_state = mask_n2s(n2s, node_state, i)
            tree = tree._replace(node2seq=new_n2s, node_state=new_node_state)
        elif train_eval and i >= base_embeddings.shape[0]:
            tree = tree._replace(node2seq=n2s, node_state=node_state)
        else:
            pass

        # conditional probs of each node relative to its siblings
        probs = model.get_probs(q_dist, tree, params, segnum, N, dist_scaling, pt_min, pt_gap).block_until_ready()
