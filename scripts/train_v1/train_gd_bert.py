...
# from protax.classify_bert import classify_file
import joblib
...
def train(tax_dir, base_dir, train_dir, targ_dir, scalings_dir, train_config, run_id, continue_training, q_perc=0.5, stop_at=1000):
    ...
    embeddings = jnp.array(np.load(base_dir)["embeddings"])
    print("base embeddings.shape: ", embeddings.shape)
    train_embeddings = jnp.array(np.load(train_dir)["embeddings"])
    print("query embeddings.shape: ", train_embeddings.shape)
    ...
    for e in range(1, train_config["num_epochs"] + 1):
        ...
        traversal = list(range(train_embeddings.shape[0]))      # Check this
        ...
        for i in tqdm(traversal, file=sys.stderr, dynamic_ncols=True, mininterval=5):
            ...
            # mask out embeddings for the query sequence
            query = train_embeddings[i]
            dists = model.seq_dist(query, embeddings)
            # dists = dists.at[i].set(1.0)
        ...
        epoch_loss = loss_sum / train_embeddings.shape[0]
        ...
        #     elif e == 40:
        #         lr = 0.01
            # print(f"next learning rate: {lr}")
            # print("--------------------------------------------")
        #     classify_file(train_dir, train_dir, f"models/model_{run_id}.npz", tax_dir, e)
...
if __name__ == "__main__":
    ...
    train(tax_dir, base_dir, train_dir, targ_dir, scalings_dir, tc, run_id, continue_training)
